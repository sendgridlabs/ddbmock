...
class SQLiteStore(object):
    ...
    def _get_by_hash_range(self, hash_key, range_key):
        request = self.conn.execute('''SELECT `data` FROM `{}`
    ...
    def _get_by_hash(self, hash_key):
        items = self.conn.execute('''SELECT * FROM `{}`
    ...
    def __setitem__(self, (hash_key, range_key), item):
        ...
        self.conn.execute('''INSERT OR REPLACE INTO `{}` (`hash_key`,`range_key`, `data`)
    ...
    def __delitem__(self, (hash_key, range_key)):
        ...
        """
        self.conn.execute('DELETE FROM `{}` WHERE `hash_key`=? AND `range_key`=?'
    ...
    def __iter__(self):
        ...
        """
        items = self.conn.execute('SELECT `data` FROM `{}`'.format(self.name))
        for item in items:
